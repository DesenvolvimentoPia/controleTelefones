...
if(empty($_GET['url'])) {
	...
	$sql = "SELECT * FROM  relatorios_usuarios_aplicativos WHERE id_aplicativo = 5 AND id_usuario = ".$_SESSION['userId'];
	...
	$sqlAcessos = "UPDATE relatorios_aplicativos SET ultimo = '".date('Y-m-d')."' WHERE id = 5";
	$resAcessos = sqlsrv_query($con, $sqlAcessos);

	$sqlAcessos = "SELECT * FROM relatorios_aplicativos_acessos WHERE id_aplicativo = 5 AND data = '".date('Y-m-d')."'";
	$resAcessos = sqlsrv_query($con, $sqlAcessos);
	...
	else $sqlAlterar = "INSERT INTO relatorios_aplicativos_acessos (id_aplicativo, data, total) VALUES (5, '".date('Y-m-d')."', 1)";
